...
from core.customer_context import CustomerAnalysisContext
...
def _show_configuration(expression_df, metadata_df=None):
    ...
    if run_analysis:
        ...
        with st.spinner(
            ...
            try:
                # Create a temporary workspace for this analysis.
                with tempfile.TemporaryDirectory(
                    prefix="onconexa_analysis_"
                ) as temp_dir:

                    temp_dir = Path(temp_dir)

                    expression_path = (
                        temp_dir / "expression.csv"
                    )

                    metadata_path = (
                        temp_dir / "metadata.csv"
                    )

                    output_path = (
                        temp_dir / "differential_expression.csv"
                    )

                    # Save the uploaded expression matrix.
                    expression_df.to_csv(
                        expression_path,
                        index=False,
                    )

                    # Use uploaded metadata when available.
                    if metadata_df is not None:
                        ...
                        # Build metadata from automatically detected
                        # sample groups.
                        generated_metadata = pd.DataFrame(
                            {
                                "sample_id": list(
                                    assessment["groups"].keys()
                                ),
                                "group": list(
                                    assessment["groups"].values()
                                ),
                            }
                        )
                    ...
                    # Translate the UI selection into the R engine's
                    # expected expression type.
                    if analysis_type == "Raw RNA-seq counts":
                        ...
                        if detected_type == "Possibly raw counts":
                            expression_type = "raw counts"
                    ...
                    # Create a persistent customer-analysis workspace.
                    from core.customer_context import (
                        create_analysis_id,
                    ...
                    # Persist the customer inputs and DE results.
                    expression_df.to_csv(
                        result_directory / "expression.csv",
                        index=False,
                    )
                    ...
                    customer_metadata = (
                        metadata_df.copy()
                        if metadata_df is not None
                        else generated_metadata.copy()
                    )

                    customer_metadata.to_csv(
                        result_directory / "metadata.csv",
                        index=False,
                    )

                    results.to_csv(
                        result_directory / "differential_expression.csv",
                        index=False,
                    )

                    # Create the central customer analysis context.
                    st.session_state[
                        ...
                        metadata=customer_metadata.copy(),
                        differential_expression=results.copy(),
                        analysis_status=status.copy(),
