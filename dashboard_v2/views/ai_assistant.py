...
available in this uploaded analysis.
...
the analysis components completed for this analysis.
...
during the stability assessment.
""".strip()
...
The current analysis contains evidence from:
...
def _show_customer_ai_assistant(customer_context):
    ...
    if (
        ...
        st.error(
            "Differential-expression results are unavailable."
        )
        return
    ...
    if "gene_name" not in differential_expression.columns:
        if "gene" in differential_expression.columns:
            ...
            st.error(
                "The differential-expression results do not "
    ...
    if st.button(
        ...
        st.success(
            "Answer generated only from the current "
            "analysis evidence."
        )
