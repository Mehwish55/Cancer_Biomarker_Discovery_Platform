...
def run_customer_stability(
    ...
    # For small candidate sets, top-10/top-20 stability can become
    # automatically inflated (for example, 10 candidates means every
    # candidate is always in the top 10). The primary stability score
    # therefore uses model importance and rank consistency instead.
    rf_importance_consistency = (
        1.0
        / (
            1.0
            + result["sd_rf_importance"]
            / result["mean_rf_importance"].replace(0, np.nan)
        )
    ).fillna(0.0)

    # Rank consistency measures how consistently a gene remains near
    # its average Random Forest rank across the repeated resamples.
    rank_sd = rf_rank_df.std(axis=0, ddof=1)

    rank_consistency = (
        1.0
        / (
            1.0
            + rank_sd / max(len(result), 1)
        )
    ).reindex(result["gene_name"]).fillna(0.0)

    result["rf_importance_consistency"] = rf_importance_consistency
    result["rank_consistency"] = rank_consistency.to_numpy()

    result["stability_score"] = (
        0.60 * result["rf_importance_consistency"]
        + 0.40 * result["rank_consistency"]
    )
